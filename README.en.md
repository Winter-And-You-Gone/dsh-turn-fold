...
- **Coupling points checklist** (check these when upgrading DSH; any failure degrades gracefully — falls back to built-in rendering / label fallbacks plus a `console.warn`, never a blank screen):
  ...
  - Locale namespace: an entry's `locale:` declaration decides which dictionary its injected `t` reads — on 0.1.2+ the official conversation dictionary lives in the `'chat'` namespace (`message.think` / `row.running` keys), on older versions in `'conversation'`; the plugin probes the official entries' declaration or `ctx.locale` at registration (`detectChatLocale`), and every `t` forwarded to official components goes through `wrapLocaleT` (misses resolve from a built-in zh/en mini dictionary instead of leaking the raw `"message.think"` key).

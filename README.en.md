...
- **Metrics update in real time**: **the duration seconds tick every second** (timed from the turn's `turn/start`), **"tokens consumed" refreshes at a randomized interval (125–250ms by default) and keeps growing**, tok/s is estimated live from output tokens / elapsed time, **the cache-hit rate shows two decimal places** (e.g. `80.00%`), and **TTFT** is approximated while running (turn start → render of the first assistant-step, seconds with one decimal), then **switches to the official persisted value once the turn ends** (the `ttftMs` carried by the turn-tail node — that turn's first request `firstTokenTime - stepStartTime`, derived from the event log, survives page reloads); once the turn ends, everything switches to the official authoritative values (turn-tail tok/s, exact `turn/end` duration);
...
- **TTFT dual source**: **while running** (turn not ended, no turn-tail yet) session snapshots carry no official timing data (`stepStartTime`/`firstTokenTime` exist only on conversation-internal event nodes), so the plugin freezes `Date.now() - turnTimings.startTime` when the first assistant-step renders (error ≈ one frame of render latency), recorded idempotently once per turn; **after the turn ends** the plugin prefers the official persisted value — the `ttftMs` carried by the turn-tail node (that turn's first step `firstTokenTime - stepStartTime`, derived by official `deriveTurnMetrics` from the event log), which survives page reloads.

...
- **指标实时更新**：大组头中的**耗时每秒走动**（从回合 `turn/start` 起计时），
  **token 随流式 usage 事件实时累计**，tok/s 按已输出 token / 已耗时实时估算，
  缓存命中率随 usage 实时计算；回合结束后全部切换为官方权威值（turn-tail 的 tok/s、
  `turn/end` 的精确耗时）；
- **"消耗token"持续外推增长**：真实 usage 只在每个请求完成时到达，两次之间数字会
  停住——运行中按观测到的生成速率（输出 token 增量 / 时间间隔；默认 30 tok/s、
  上限 300 tok/s，见 `CONFIG.liveTokenRate*`）持续外推增长，保持"实时消耗"的观感；
  新 usage 到达时立即校正为真实值（可能回跳，属预期）；
- **滚轮式数字动画**：运行中数值变化时，每一位数字独立"滚动"到新值（里程表/滚轮效果，
  350ms 回弹缓动；耗时每秒滚动一次，token/tok/s/缓存命中随数据到达滚动）——数字拆成
  逐位视窗、内部竖排 0-9，视觉上像计数滚筒；完整文案另有 sr-only 副本，读屏/无障碍不受影响，
...
- 整回合折叠通过会话快照的 `turnEnds`（turn/end 事件驱动）判定回合完成，配合
  ...
  大组头即出现：耗时用每秒时钟补 `Date.now()` 实时走动，token 随流式 `usage` 事件
  （assistant/chunk 的 usage 块）实时累计，全部指标在 `turn/end` 后切换为权威值。
